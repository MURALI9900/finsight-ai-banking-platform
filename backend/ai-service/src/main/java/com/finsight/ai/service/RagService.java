...
public class RagService {
    private final List<RagDocument> documents = List.of(
        new RagDocument("POL-001", "Loan Repayment Policy",
            "Loan repayments may be rejected when the available account balance is insufficient. "
          + "Officers should verify the repayment amount, available balance, loan status, and transaction reference."),
        new RagDocument("POL-002", "Overdue Loan Operations",
            "Loans overdue beyond 30 days should be reviewed by an authorized officer. "
          + "The officer should verify outstanding amount, overdue days, customer status, and applicable recovery workflow."),
        new RagDocument("POL-003", "Failed Transaction Handling",
            "For a failed transaction, officers should review the failure reason and transaction timestamp. "
          + "Core banking timeout failures should be checked against the core banking service status before retrying.")
    );
