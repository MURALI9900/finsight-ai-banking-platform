...
public class QueryIntentService {
 ...
 public QueryIntentService(BankingToolClient tools){this.tools=tools;}
 public QueryResult answer(String question){
  ...
  if(q.contains("overdue")||q.contains("over due")){
   ...
   return new QueryResult("Found "+data.size()+" loan(s) overdue by more than "+days+" days.","OVERDUE_LOANS","getOverdueLoans",data);
  }
  if(q.contains("failed")&&(q.contains("transaction")||q.contains("payment"))){
   var data=tools.failedTransactions();
   return new QueryResult("Found "+data.size()+" failed transaction(s).","FAILED_TRANSACTIONS","getFailedTransactions",data);
  }
  if(q.contains("summary")||q.contains("total")||q.contains("overview")){
   var data=tools.summary();
   return new QueryResult("Here is the current banking operations summary.","BANKING_SUMMARY","getBankingSummary",List.of(data));
  }
