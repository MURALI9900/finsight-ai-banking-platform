...
public class AiController {
 ...
 public AiController(QueryIntentService queryService){this.queryService=queryService;}
 @PostMapping("/ask")
 public AskResponse ask(@Valid @RequestBody AskRequest request){
  var result=queryService.answer(request.question());
  return new AskResponse(result.answer(),result.intent(),result.toolUsed(),result.data());
 }
}